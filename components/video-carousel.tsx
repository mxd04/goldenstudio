...
const PARTICLE_COUNT = typeof window !== 'undefined' && window.innerWidth < 768 ? 6 : 12;
...
const MagicBentoCard: React.FC<MagicBentoCardProps> = React.memo(({ children, className = '' }) => {
  ...
  const shouldAnimateParticles = !prefersReducedMotion && !isMobile;
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      if (!shouldAnimateParticles) return; // Skip click effects on mobile/reduced motion
  ...
  }, [animateParticles, clearAllParticles, shouldAnimateParticles]);
...
export default function HaircutsSection() {
  ...
  useEffect(() => {
    ...
    // DISABLE spotlight effect on mobile - too heavy
